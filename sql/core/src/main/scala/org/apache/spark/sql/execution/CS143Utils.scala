...
object CS143Utils {
  ...
  def getListFromBytes(bytes: Array[Byte]): JavaArrayList[Row] = {
    ...
    try {
      ...
      while ((temp = in.readObject() match {
        ...
        case _: Throwable => throw new RuntimeException ("Unexpected casting exception while reading from file.")
      }) != null) {
  ...
  /**
    ...
    * @return
    */
  def getNextChunkBytes(inStream: InputStream, nextChunkSize: Int,
                        previousArray: Array[Byte] = null): Array[Byte] = {
  ...
  def generateCachingIterator(
    ...
    udf match {
      ...
      operation. Note that this is very similar to Project in
      ...
      case u: ScalaUdf => {
        ...
        CachingIteratorGenerator(udf.children, udf,
          preUdfExpressions, postUdfExpressions, inputSchema)
      }
    }
...
object CachingIteratorGenerator {
  ...
  def apply(
             ...
             inputSchema: Seq[Attribute]): (Iterator[Row] => Iterator[Row]) = {
    // this is actually a higher order function (definition of above)
    { input =>
      new Iterator[Row] {
